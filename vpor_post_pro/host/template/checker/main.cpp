...
// #                                                          #
// # Sven Gesper, IMS, Uni Hannover, 2021                     #
// ############################################################
...
int main(int argc, char **argv) {
    ...
    parser.setApplicationDescription("Hardware Template App for VPRO Functions");
 ...
 */
    ...
    console.printf_normal("4. Receiving Addresses for Start Flag...\n");
    MM_START_FLAG = get_debug_fifo_entry();
    console.printf_normal("\tMM_START_FLAG: %i [0x%x]\n", MM_START_FLAG, MM_START_FLAG);

    console.printf_normal("5. Receiving Addresses for Running Flag...\n");
    MM_RUNNING_FLAG = get_debug_fifo_entry();
    console.printf_normal("\tMM_RUNNING_FLAG: %i [0x%x]\n", MM_RUNNING_FLAG, MM_RUNNING_FLAG);

    console.printf_normal("6. Receiving Addresses for Return Flag...\n");
    MM_RETURN_FLAG = get_debug_fifo_entry();
    console.printf_normal("\tMM_RETURN_FLAG: %i [0x%x]\n", MM_RETURN_FLAG, MM_RETURN_FLAG);
    ...
    auto comm = new Communication(mutex, *fifoReader, console, MM_TEST_RESULT, MM_TEST_DATA_1, MM_TEST_DATA_2,
                                  MM_START_FLAG,
                                  MM_RUNNING_FLAG, MM_RETURN_FLAG,
                                  executable, input_a, input_b, output, num_cl, num_vu_per_cl, silent, testnr, a);

    // finished -> quit ?
    QCoreApplication::connect(comm, SIGNAL(finished()), a, SLOT(quit()));
    QCoreApplication::connect(comm, SIGNAL(finished()), mipsDebug, SLOT(quit()));
    QCoreApplication::connect(comm, SIGNAL(finished()), fifoReader, SLOT(quit()));
    QCoreApplication::connect(comm, SIGNAL(finished()), &console, SLOT(quit()));
    comm->start();
