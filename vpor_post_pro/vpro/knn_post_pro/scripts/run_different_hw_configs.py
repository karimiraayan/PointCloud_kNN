...
#	os.mkdir("log")
try: 
	shutil.rmtree("/localtemp/karimi/log")
	os.makedirs("/localtemp/karimi/log")
except: 
	print("error create log folder in localtemp") 
	exit()
try: 
	shutil.rmtree("/localtemp/karimi/error_log")
	os.makedirs("/localtemp/karimi/error_log")
except: 
	print("error create error_log folder in localtemp") 
	exit()
try: 
	shutil.rmtree("/localtemp/karimi/builds")
	os.makedirs("/localtemp/karimi/builds")
except Exception as e: 
	print("error create builds folder in localtemp") 
	print(e)
	exit()
try: 
	#os.makedirs("/localtemp/karimi/builds/init")
	shutil.copytree("../init", "/localtemp/karimi/builds/init")
except Exception as e: 
	print("error copy init folder to localtemp")
	print(e)
	exit()
try: 
	#os.makedirs("/localtemp/karimi/builds/exit")
	shutil.copytree("../exit/", "/localtemp/karimi/builds/exit/")
except Exception as e: 
	print("error copy exit folder to localtemp") 
	print(e)
	exit()
try: 
	#os.makedirs("/localtemp/karimi/builds/data")
	shutil.copytree("../data/", "/localtemp/karimi/builds/data/")
except Exception as e: 
...
while (not (len(configlist) == 0)) or (len(runnings) > 0):
	...
	for i in range(0, len(runnings)):
		if runnings[i][2].poll() != None: # process is not alive
			...
			shutil.rmtree("/localtemp/karimi/builds/build_release_c"+str(runnings[i][0])+"u"+str(runnings[i][1]), ignore_errors=True)
			print("Removing: /localtemp/karimi/builds/build_release_c"+str(runnings[i][0])+"u"+str(runnings[i][1]))
			...
			try: shutil.copyfile("/localtemp/karimi/builds/data/statistic_out.csv", "./results.csv") 
	...
	for i in range (0, min(maxParallelRuns, freeRuns)):
		if len(configlist) > 0:
			...
			c = config[0]
			u = config[1]
			process = subprocess.Popen(["make", "build_release=/localtemp/karimi/builds/build_release_c"+str(c)+"u"+str(u), "COMMENT=\"CNN_MOBILENETV2\"", "CLUSTERS="+str(c), "UNITS="+str(u), "scripted"], 
				cwd="../", stdout=open("/localtemp/karimi/log/C"+str(c)+"U"+str(u)+".log", "w"), stderr=open("/localtemp/karimi/error_log/C"+str(c)+"U"+str(u)+".log", "w"))
			starts += 1
