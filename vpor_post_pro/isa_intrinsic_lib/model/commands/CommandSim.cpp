...
CommandSim::CommandSim(CommandSim *ref) :
    ...
    switch(type){
        ...
        case BIN_FILE_DUMP:
            str += QString("Addr: %1 (%2 bytes) from file: %3")
                    .arg(addr).arg(num_bytes).arg(file_name);
            break;
        case BIN_FILE_RETURN:
            str += QString("Addr: %1 (%2 bytes)")
                    .arg(addr).arg(num_bytes);
            break;
        case AUX_MEMSET:
            str += QString("Addr: %1 (%2 bytes) to value %3")
                    .arg(addr).arg(num_bytes).arg(value);
            break;
        case DUMP_LOCAL_MEM:
