...
QWidget *createunitwidget(CommandWindow *mainwindow,int unitqueue,int u,int lane_cnt,QVector<std::shared_ptr<CommandVPRO>> commands,QVector<QVector<std::shared_ptr<CommandVPRO>>> command_queue,bool paused,int *command,QVector<bool> isLoopparsing,QVector<bool> isLopppush,int c){
    ...
    QObject::connect(printbutton, &QPushButton::clicked,[command_queueforprint,c,u]{printcommandqueue(command_queueforprint,c,u);});
...
QScrollArea *createcommandqueue(QVector<QVector<std::shared_ptr<CommandVPRO>>> command_queue,CommandWindow *mainwindow,int unitqueue){
    ...
    for (auto vprocommand : command_queue[unitqueue]) {
        ...
        QObject::connect(item, &QPushButton::clicked, [mainwindow,vprocommand]() {createtablewindow(mainwindow,vprocommand);});
...
QGridLayout *createlaneslayout(int lane_cnt,int *command,QVector<std::shared_ptr<CommandVPRO>> commands,CommandWindow *mainwindow){
    ...
    for(int l=0; l<lane_cnt; l++){
        ...
        QObject::connect(item, &QPushButton::clicked, [mainwindow,cmd]() {createtablewindow(mainwindow,cmd);});
...
void printcommandqueue(QVector<std::shared_ptr<CommandVPRO>> command_queue,int c,int u){
    qDebug()<<"\n";
    qDebug().noquote()<<"Printing Commandqueue for Unit "+QString::number(u)+" in Cluster "+QString::number(c);
    int i=0;
    for (auto command:command_queue){
        ...
        if (command->src1.sel == SRC_SEL_ADDR){
            Src1=(QString::number(command->x*command->src1.alpha+command->y*command->src1.beta+command->src1.offset));
        }
        else if(command->src1.sel == SRC_SEL_IMM){
            Src1=("Imm "+QString::number(command->src1.imm));
        }
        else if (command->src1.sel == SRC_SEL_IMM){
            Src1=("chain left");
        }
        ...
        if (command->src2.sel == SRC_SEL_ADDR){
            Src2=(QString::number(command->x*command->src2.alpha+command->y*command->src2.beta+command->src2.offset));
        }
        else if(command->src2.sel == SRC_SEL_IMM){
            Src2=("Imm "+QString::number(command->src2.imm));
        }
        else if (command->src2.sel == SRC_SEL_IMM){
            Src2=("chain left");
        }
        ...
        qDebug().noquote()<<"   Mask: "+QString::number(command->id_mask)+" Sourceadress 1: "+Src1+" Sourceadress 2:"+Src2;
        i++;
    }
