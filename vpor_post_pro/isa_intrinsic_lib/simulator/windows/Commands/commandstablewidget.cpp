...
{                                              //constructor creates matrix with the data of the command, for creating the table ->void createtable
    ui->setupUi(this);
    tabledata.resize(2);
    ...
    QTableWidgetItem *Destinationadressvalue=new QTableWidgetItem(QString::number(cmd->x*cmd->dst.alpha+cmd->y*cmd->dst.beta+cmd->dst.offset));
    ...
    if (cmd->src1.sel == SRC_SEL_ADDR){
        Sourceadress1value->setText(QString::number(cmd->x*cmd->src1.alpha+cmd->y*cmd->src1.beta+cmd->src1.offset));
        tabledata[1].append(Sourceadress1value);
    }
    else if(cmd->src1.sel == SRC_SEL_IMM){
        Sourceadress1value->setText("Imm: "+QString::number(cmd->src1.imm));
        tabledata[1].append(Sourceadress1value);
    }
    else if (cmd->src1.sel == SRC_SEL_IMM){
    ...
    if (cmd->src2.sel == SRC_SEL_ADDR){
        Sourceadress2value->setText(QString::number(cmd->x*cmd->src2.alpha+cmd->y*cmd->src2.beta+cmd->src2.offset));
        tabledata[1].append(Sourceadress2value);
    }
    else if(cmd->src2.sel == SRC_SEL_IMM){
        Sourceadress2value->setText("Imm: "+QString::number(cmd->src2.imm));
        tabledata[1].append(Sourceadress2value);
    }
    else if (cmd->src2.sel == SRC_SEL_IMM){
...
void commandstablewidget::createtable(){
    ...
    this->setFixedSize((2*commandstable->columnWidth(0)*2)-110,commandstable->rowHeight(0)*commandstable->rowCount()+21);
}
